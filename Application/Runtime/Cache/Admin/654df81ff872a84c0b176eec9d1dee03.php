        ...
        <div class="layui-field-box">
            ...
            <table class="layui-table" lay-even>
                ...
                <?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td>
                            <?php if($vo['status']): ?><span class="layui-btn layui-btn-danger confirm" data-info="你确定禁用当前API么？" data-id="<?php echo ($vo['id']); ?>" data-url="<?php echo U('close');?>">禁用</span>
                                <?php else: ?>
                                <span class="layui-btn confirm" data-info="你确定启用当前API么？" data-id="<?php echo ($vo['id']); ?>" data-url="<?php echo U('open');?>">启用</span><?php endif; ?>
                            <span data-url="<?php echo U('FieldsManage/request', array('hash' => $vo['hash']));?>" class="layui-btn field layui-btn-primary">请求参数</span>
                            <span data-url="<?php echo U('edit', array('id' => $vo['id']));?>" class="layui-btn edit layui-btn-normal">编辑</span>
                            <span data-url="<?php echo U('FieldsManage/response', array('hash' => $vo['hash']));?>" class="layui-btn field layui-btn-warm">返回参数</span>
                            <span class="layui-btn layui-btn-danger confirm" data-id="<?php echo ($vo['id']); ?>" data-info="你确定删除当前API么？" data-url="<?php echo U('del');?>">删除</span>
                        </td>
